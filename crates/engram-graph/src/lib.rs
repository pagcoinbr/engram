...
impl GraphClient {
    ...
    pub async fn native_keyword_files(
        ...
        let tokens = query
            ...
            .collect::<Vec<_>>();
        self.file_hits("MATCH (m:EngramMemory)-[:HAS_FACT]->(f:EngramFact) WHERE f.valid_until IS NULL AND any(token IN $tokens WHERE toLower(f.text) CONTAINS token) WITH m, collect(DISTINCT f.text) AS fact_text, count(f) AS fact_score OPTIONAL MATCH (m)-[:HAS_TRIPLE]->(t:EngramTriple {status: 'active'}) WHERE t.valid_until IS NULL AND any(token IN $tokens WHERE toLower(t.subject + ' ' + t.relation + ' ' + t.object) CONTAINS token) WITH m, fact_text, fact_score, collect(DISTINCT t.subject + ' ' + t.relation + ' ' + t.object) AS triple_text, count(t) AS triple_score WITH m, fact_text + triple_text AS texts, fact_score + triple_score AS score RETURN m.file AS file, texts AS facts, score ORDER BY score DESC LIMIT $limit", serde_json::json!({"tokens": tokens, "limit": limit})).await
    }
