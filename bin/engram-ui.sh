...
export ENGRAM_VECTOR="${ENGRAM_VECTOR:-$CLAUDE/vector}"
...
# Pick the interpreter with the most backends so every tab works:
#   graph venv  — graphiti/neo4j (+ qdrant-client when --vector) → covers all tabs.
#   vector venv — qdrant-client (vector-only installs; the Graph tab degrades).
#   system python3 — last resort (Graph/Vector tabs show "unavailable").
PY="$CLAUDE/graph/venv/bin/python"
[[ -x "$PY" ]] || PY="$CLAUDE/vector/venv/bin/python"
